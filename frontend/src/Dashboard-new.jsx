  ...
  return (
    <div style={{ fontFamily: 'Poppins, sans-serif' }} className="min-h-screen bg-white flex flex-col">
      <div className="flex-1 flex flex-col">
        ...
        {/* Content */}
        <main className="max-w-6xl mx-auto px-6 lg:px-8 py-10 pb-20 md:pb-10 w-full flex-1">
                ...
                <>
                  {myEvents.length === 0 ? (
                    <div className="bg-gray-50 rounded-2xl border border-gray-200 p-12 flex flex-col items-center gap-4 text-center">
                      ...
                      <div>
                        <p className="text-lg font-semibold text-gray-900">No events created yet</p>
                        ...
                        return (
                          <div key={event.id} className="bg-white rounded-2xl border border-gray-200 overflow-hidden hover:shadow-md transition-shadow group">
                            <div className="bg-gradient-to-br from-blue-400 to-blue-600 h-20 relative flex items-end justify-end p-3">
                              <span className={`inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-1 rounded-full bg-white/90 ${c.pill}`}>
                                <span className={`w-1.5 h-1.5 rounded-full ${c.dot}`} />
                                {c.label}
                              </span>
                            </div>
                            <div className="p-4">
                              <h3 className="font-semibold text-gray-900 mb-2 group-hover:text-blue-600 transition-colors line-clamp-2">{event.name}</h3>
                ...
                <>
                  {joinedEvents.length === 0 ? (
                    <div className="bg-gray-50 rounded-2xl border border-gray-200 p-12 flex flex-col items-center gap-4 text-center">
                      ...
                      <div>
                        <p className="text-lg font-semibold text-gray-900">No events joined yet</p>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {joinedEvents.map(event => (
                        <div key={event.id} className="bg-white rounded-2xl border border-gray-200 overflow-hidden hover:shadow-md transition-shadow cursor-pointer group" onClick={() => navigate(`/event/${event.id}/directory`)}>
                          ...
                          <div className="p-4">
                            <h3 className="font-semibold text-gray-900 mb-2 group-hover:text-blue-600 transition-colors">{event.name}</h3>
          ...
          {activeTab === 'profile' && (
            <>
              <div className="mb-8">
                ...
                <p className="text-gray-600">Manage your information and settings</p>
              </div>

              {/* Main profile card */}
              <div className="bg-white rounded-2xl border border-gray-200 p-6 mb-6">
                <div className="flex items-center gap-5 mb-6">
                  <div className="w-16 h-16 bg-gradient-to-br from-blue-400 to-blue-600 rounded-2xl flex items-center justify-center shrink-0">
                    <span className="text-2xl font-bold text-white">
                      {(userData?.name || 'U').charAt(0).toUpperCase()}
                    </span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="font-bold text-gray-900 text-lg truncate">{userData?.name || 'User'}</p>
                ...
                <div className="grid grid-cols-3 gap-3 mb-6">
                  <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl p-4 text-center">
                    ...
                    <p className="text-sm font-bold text-blue-700">
                      {userData?.role === 'SUPER_ADMIN' ? '👑' : userData?.role === 'ORGANIZER' ? '🎯' : '👤'}
                    </p>
                  </div>
                  <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-xl p-4 text-center">
                    <p className="text-xs text-gray-600 font-medium mb-1">Hosting</p>
                    <p className="text-sm font-bold text-green-700">{myEvents.length}</p>
                  </div>
                  <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl p-4 text-center">
                    <p className="text-xs text-gray-600 font-medium mb-1">Joined</p>
                    <p className="text-sm font-bold text-purple-700">{joinedEvents.length}</p>
                  </div>
                </div>

                {userData && (userData.designation || userData.company || userData.linkedin_url) && (
                  <div className="space-y-3 pt-6 border-t border-gray-100">
                    {userData.designation && (
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-gray-600">Designation</span>
                        <span className="text-sm font-semibold text-gray-900">{userData.designation}</span>
                      </div>
                    )}
                    {userData.company && (
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-gray-600">Company</span>
                        <span className="text-sm font-semibold text-gray-900">{userData.company}</span>
                      </div>
                    )}
                    {userData.linkedin_url && (
                      <div className="flex justify-between items-center">
                        <span className="text-sm text-gray-600">LinkedIn</span>
                        <a href={userData.linkedin_url} target="_blank" rel="noopener noreferrer" className="text-sm font-semibold text-blue-600 hover:underline">
                          Visit Profile →
                        </a>
                      </div>
                    )}
              ...
              <div className="space-y-3">
                <button
                  ...
                  className="w-full bg-white rounded-2xl border border-gray-200 p-4 flex items-center gap-3 hover:border-blue-600 hover:bg-blue-50 transition-colors"
                >
                ...
                {userData?.role === 'PARTICIPANT' && (
                  <button
                    ...
                    className="w-full bg-white rounded-2xl border border-gray-200 p-4 flex items-center gap-3 hover:border-amber-600 hover:bg-amber-50 transition-colors"
                  >
                    ...
                    <div className="flex-1 text-left">
                      <p className="text-sm font-semibold text-gray-900">Become an Organizer</p>
                      <p className="text-xs text-gray-500">Request organizer privileges</p>
                    </div>
                    <svg className="w-4 h-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                    </svg>
                ...
                <button
                  ...
                  className="w-full bg-white rounded-2xl border border-gray-200 p-4 flex items-center gap-3 hover:border-red-600 hover:bg-red-50 transition-colors"
                >
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 z-20">
        <div className="flex items-end px-6 pb-2 pt-2">
          <button
          ...
          >
            <svg className="w-6 h-6" fill={activeTab === 'events' ? 'currentColor' : 'none'} viewBox="0 0 24 24" stroke="currentColor" strokeWidth={activeTab === 'events' ? 0 : 2}>
          ...
          >
            <svg className="w-6 h-6" fill={activeTab === 'profile' ? 'currentColor' : 'none'} viewBox="0 0 24 24" stroke="currentColor" strokeWidth={activeTab === 'profile' ? 0 : 2}>
          ...
          >
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 bg-blue-600 text-white text-sm font-semibold py-3.5 rounded-2xl hover:bg-blue-700 transition-colors"
              >
                Copy Link
              </button>
              <button
                onClick={() => setShowQR(false)}
                className="flex-1 bg-gray-100 text-gray-700 text-sm font-semibold py-3.5 rounded-2xl hover:bg-gray-200 transition-colors"
              >
                Close
              </button>
            </div>
